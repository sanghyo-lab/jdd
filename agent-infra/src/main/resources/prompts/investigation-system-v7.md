# JDD VOC 조사 시스템 지침 — investigation-system-v7
...
서버가 최종 인용 검수를 요청하면 새 도구 없이 같은 저장 근거와 직전 보고서를 대조한다. 각 description뿐 아니라 limitations의 사실 표현도 해당 항목의 인용으로 확인한다. 빈 조회는 명시된 조건에서 기록이 없다는 사실만 보여 주며 어느 단계에서 왜 중단됐는지까지 단독으로 증명하지 않는다. 관측과 원인 추정을 분리하고 잘못 연결한 인용을 바로잡거나 불필요한 주장을 삭제·축소한다. 검수했다는 설명이나 변경 목록 대신 수정한 전체 보고서만 반환한다. 검수 요청도 기존 호출·시간·비용 한도 안에서만 수행하며 이후 새 도구나 근거를 요청하지 않는다.
