...
사용자는 데모 비용에 GS 해커톤 OpenAI API 프로모션 크레딧을 사용하는 방향으로 변경했다. 프로모션 코드를 API 키로 설정하지 마라.
사용자가 전달한 Applied promotions 표기에 따르면 $50.00 크레딧은 2026년 9월 21일에 적용됐다. 이 날짜는 적용일이며 만료일이 아니다. 적용 금액을 현재 잔액으로 간주하지 마라.
실제 잔액·만료일·적용 조직/프로젝트를 확인하고 그 조직/프로젝트의 API 키를 사용한다. 기존 키의 적용 여부는 확인 전까지 미정으로 둔다.
$50 크레딧을 사용 예산 확대 허가로 해석하지 말고 아래 $30 초과 예상 시 별도 처리 기준을 유지하라.
사용 범위는 서비스 데모로 제한한다. 키와 프로모션 코드 원문은 저장소·문서·로그에 기록하지 말고 API 키만 실제 데모 실행 환경의 비밀 설정으로 주입하라.
...
데모 제공자는 OpenAI이며 프로모션 적용 조직·프로젝트의 서비스용 API 키를 사용한다. 개발 경로는 이 키를 자동으로 읽거나 호출하지 않는다. 모델명·실제 접근은 미검증이다.
데모는 로컬 앱 + ngrok로 구성한다. 로컬 Agent가 OpenAI API를 직접 호출하며 ngrok는 web의 외부 접속에만 사용한다.
API 연동 계층은 분리하되 이 구성을 위해 별도 LLM 중계 서버를 신설하거나 로컬 LLM으로 전환하지 마라.
