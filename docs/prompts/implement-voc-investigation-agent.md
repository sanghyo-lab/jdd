...
문서 상태: **최종 검수용 후보, 사용자 확정 대기**.
[검수 기록](../planning/han-jaehong-prompt-review.md)의 확인 범위·남은 실행 조건을 검토한 뒤 사용자가 확정한다.
이번 문서 정리·공유는 서비스 구현 시작이나 유료 모델 호출 승인을 뜻하지 않는다.
...
**서비스 LLM 연결을 조사 로직과 분리한다.** [실행 계약](../llm-runtime.md)의 명시적 local/codex_oauth,
deployed/openai_api, test/mock만 사용한다. 기존 InvestigationModel 공통 port와 두 직접 HTTP Responses 어댑터를 유지한다.
Codex SDK·추론별 CLI 실행·별도 범용 프록시를 추가하지 않는다. APP_RUNTIME/LLM_PROVIDER 누락·잘못된 조합은 시작 오류다.

로컬 개발·수동 반복·영상 데모는 CODEX_MODEL과 프로젝트 전용 CODEX_AUTH_FILE을 사용한다. 공식 CLI로 최초 로그인하고
저장소 밖 별도 디렉터리에 file auth를 저장한다. 기존 개발용 auth.json을 복제하지 않는다. 앱은 매 요청 최신 파일을 읽고
refresh를 재구현하지 않는다. 만료/401/403은 재로그인 명령을 안내하며 API key로 fallback하지 않는다.

배포와 배포 미리보기는 OPENAI_MODEL·OPENAI_API_KEY secret·JDD_AGENT_API_PROFILE을 명시한다. OAuth 파일은 읽지 않는다.
$50 프로모션 API 크레딧은 배포에만 사용하고 기존 누적 $30 초과 예상 시 별도 처리 조건·영속 비용 장부를 유지한다.
로컬 OAuth 사용량은 별도 관측하며 API USD로 환산하지 않는다. ngrok 비용도 별도다.

공통 텍스트·대화·strict schema·서버 function calls를 보존한다. SSE의 chunk/UTF-8 경계, 실패·불완전 종료·취소를 처리한다.
버전 프롬프트와 저장된 실제 근거만 모델에 전달한다. 어댑터가 도구를 자동 실행하지 않고 기존 실행기가 허용된 도구만 실행한다.
Codex backend와 범용 API의 지원/호환성을 동일하다고 가정하지 않는다. 로컬 모델 지원이나 워크스페이스 무제한 혜택을 추측하지 않는다.

자동 테스트는 networkless mock 또는 합성 loopback HTTP만 사용한다. 일반 up/check/publish에 live 호출/터널을 연결하지 않는다.
로그인/모델 접근 준비 후 local smoke를 별도 실행하고 실제 VOC 도구·한국어 보고서 품질 검증과 구분한다.
배포 smoke는 배포 환경에서만 수행한다. 실제 검증을 못 했다면 미검증으로 기록하며 DONE을 쓰지 않는다.
...
저장소의 Java 21·Gradle Wrapper·Spring Boot 설정을 따르고 기존 조사 port에 직접 HTTP Responses 어댑터를 연결하라.
...
각 PC에서는 로컬의 세 앱·DB를 연결한다. 데모 PC에서는 web도 로컬로 실행하고 ngrok로 web 진입점 하나만 연결한다.
브라우저는 같은 출처의 web API를 호출하고 web 서버가 로컬 VOC·commerce에 중계한다. VOC → Agent는 Compose 내부 주소를 유지한다.
ngrok 공개 URL을 AGENT_BASE_URL·COMMERCE_BASE_URL·모델 API 주소로 사용하지 마라. Agent·DB·관리 경로는 직접 공개하지 않는다.
OpenAI 키는 로컬 Agent 실행 환경에서만 사용하고 ngrok authtoken과 분리한다. 모델 호출·비용은 기존 데모 정책을 따른다.
[ngrok 로컬 데모 절차](../ngrok-local-demo.md)에 따라 web·접근 제어·터널 중단/재연결·상태/근거 조회를 김아름과 검증하라.
...
로컬 앱 + ngrok + OpenAI 직접 호출을 사용한다. 모델명·접근 가능한 API 설정·ngrok 공개 URL·실행 한도의 구체적인 값은 미확정/미검증이다.
