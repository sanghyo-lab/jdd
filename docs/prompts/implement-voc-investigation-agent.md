...
현재는 프롬프트 준비 단계다. 문서 작성·검토 요청만으로 구현을 시작하지 않는다.
사용자가 이 파일을 기준으로 구현을 시작하라고 요청하면 아래 지시를 수행한다.
...
완성 기준은 문의 한 건이 실제 모델·도구 호출을 거쳐 검증된 결과와 근거 조회까지 이어지는 것이다.
...
제공자·모델·주소·인증 방식은 아직 미정이다. 별도 배포할 LLM 중계 서버가 필요한지도 미정이다.
...
현재 LLM 제공자·API 주소·모델명·인증 방식·별도 중계 서버 필요 여부와 실행 한도의 구체적인 값은 미정이다.
