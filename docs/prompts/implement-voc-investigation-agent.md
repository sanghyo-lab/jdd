...
- [프로젝트 작업 규칙](../../AGENTS.md), [협업 절차](../autonomous-development.md), [로컬 실행](../local-development.md)
...
| 담당 | 책임 |
| --- | --- |
| 한재홍 | `agent-app/`, `agent-core/`, `agent-infra/`, `docs/status/agent.md`: 조사 API·실행·도구·근거·보고서 |
...
사용자가 확정한 제공자는 OpenAI이며, 서비스용 OpenAI LLM API 키는 별도로 제공될 예정이다.
키가 이미 전달되거나 호출 검증이 끝난 것으로 간주하지 마라. 모델명·실제 API 주소·계정의 모델 접근은 설정 및 수령 후 검증으로 확정한다.
키 수령 전에도 OpenAI 연동 계층·설정 검증·도구 실행·근거 저장·보고서 검증을 구현하고 실제 모델 평가는 미검증으로 남긴다.
...
LLM API 연결 완료는 다음 실제 검증으로 판단하라.
...
제공자는 OpenAI, 인증은 별도 제공될 서비스용 API 키를 사용한다. 모델명·실제 API 주소·별도 LLM 중계 서버 필요 여부는 미정이다.
...
| 단계 | 산출물과 통과 기준 |
| --- | --- |
| 1. 현재 상태와 LLM API 준비 | 기존 변경·계약·상대 전달물 확인. LLM API 주소·모델·인증 방식·연동 계층과 미정 사항 정리 |
| 2. LLM API 연결과 조사 API | 실제 모델의 도구 호출·구조화 출력·연결 실패 처리 확인. 접수·조회·저장·중복·충돌 계약 검증. 외부 연결이 준비되지 않으면 독립 작업부터 진행하고 실제 검증은 미완료로 기록 |
| 3. VOC-07 전체 흐름 | 재고·로그·소스·정책의 실제 도구 결과로 분석하고 VOC에서 결과·근거 조회 |
| 4. 7개 조사 확장 | 나머지 주문·결제·쿠폰·취소 조사도 같은 흐름·도구로 처리 |
| 5. 정상·실패·복구 | 정상 쿠폰 거절·정상 주문, 대상 불명확, 누락 근거, 도구 실패, 시간 초과, 잘못된 보고서, 재시작 검증 |
| 6. 통합·인계 | 최신 상대 API 연결, 실행 명령·설정·모델·조사 ID·buildId·성공/실패 자료 제공 |
...
- 모델의 가짜 근거 참조·잘못된 형식 거절, 예산 소진과 늦은 응답의 상태 보존.
...
OpenAI와 별도 API 키 제공 계획은 확정됐다. 키 수령·접근 검증은 아직 완료되지 않았다.
