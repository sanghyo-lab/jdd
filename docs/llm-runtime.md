...
두 경로 모두 `parallel_tool_calls=true`로 독립 읽기를 한 모델 응답에 묶을 수 있다. 실제 도구는 기존 실행기가 검증해 순서대로 실행·저장한다. 서버는 남은 모델/도구 횟수를 알리고 마지막 호출이나 도구 한도 소진 뒤에는 `tool_choice=none`으로 보고서만 요청한다. 기본 8회 모델·24회 도구·3분 조사 상한과 비용 장부는 그대로다.

`investigation-system-v9`는 원인·조치·예방이 있는 보고서의 최종 인용 검수 응답 1회를 같은 한도 안에 확보하게 한다. 구조 검증된 초안은 아직 저장/공개하지 않고 같은 모델로 검수한다. 검수 전용 요청에는 초안·직접 인용한 CODE 원문·저장된 DATA/LOG/POLICY를 한 번씩 전달한다. 이전 모델의 추정/암호화된 reasoning 이력과 인용하지 않은 CODE 검색 구간은 검수 입력에서 제외한다. 원본 근거를 삭제하거나 내용·출처를 바꾸지 않으며 제외 범위도 요청에 명시한다. 일반 도구 조사에서는 기존 전체 대화/도구 이력을 유지한다.

공통 지침과 일반 보고서/최종 부분 검수 지침을 별도 버전 리소스로 관리한다. 서버의 reviewDraft 유무로 해당 단계의 지침만 실제 instructions에 넣으며 문의 문자열로 단계를 선택하지 않는다. 일반 검증 오류에는 기존 사실·인용까지 포함한 전체 보고서를 요청하고, 부분 검수 오류에는 아직 변경되지 않은 원래 초안을 기준으로 필요한 수정만 요청한다. v9의 prompt SHA-256은 일반 지침 완성 문자열 + NUL + 검수 지침 완성 문자열의 UTF-8 바이트를 고정 순서로 계산한 묶음 해시다. 호출 수나 형식/인용 검증 기준을 줄이지 않는다.

검수 응답 `investigation_report_review`는 수정할 기존 facts/hypotheses/actions/prevention 항목만 같은 id와 전체 필드로 반환한다. 빈 배열은 해당 항목들을 그대로 유지하며, summary=null은 기존 요약 유지, removeItemIds는 기존 항목 삭제다. 다른 구역/없는 id·중복 수정·동일 항목의 수정/삭제 충돌은 거절한다. 검수 결과를 기존 초안에 적용한 뒤 전체 보고서를 같은 서버 검증으로 다시 확인하므로 없는 근거·필수 직접 인용 누락·확인 사실 전체 삭제를 우회할 수 없다. 미검수 초안을 실패의 대체 결과로 반환하지 않는다. 이 형식은 모델 내부용이며 기존 VOC·web 보고서 API는 그대로다.

검수 시작 후 도구는 제공/실행하지 않는다. 기존 보정 횟수·모델/도구/시간 상한·사용량/비용 장부를 유지한다. 단순 사실만 있는 보고서나 입력 부족에는 추가 검수를 강제하지 않으며 v1~v8 리소스를 보존한다.

검수는 각 항목의 description/limitations와 자체 인용을 대조하고 관측·인과 추정을 구분한다. 빈 조회만으로 특정 중단 단계나 후속 처리 불필요를 단정하지 않도록 지시한다. 평가 정답은 넣지 않는다. 같은 모델의 재검수는 의미적 정확성을 보장하지 않으므로 실제 보고서의 품질은 별도로 확인한다. 검수 호출의 토큰·지연·배포 비용도 기존 장부에 포함되며 추가 호출의 품질 개선/효율은 실측 전 단정하지 않는다.
...
개발 진단 로그에는 응답의 final/unphased/commentary 메시지 수와 도구 수, 최종 검수 요청/적용의 조사 ID·반복 번호·항목 교체/삭제 수·요약 제공 여부, 보고서 검증 실패의 조사 ID·모델 반복 번호·서버 정의 사유(중복 제외 최대 16개)만 남긴다. 모델 원문·근거 값·인증 정보·예외 원문은 이 로그에 넣지 않는다. 검증 기준과 오류 DTO는 그대로이며 일반 사용자 화면에 구현 정보를 추가하지 않는다.

현재 조사에서 서버가 저장해 요청에 포함한 근거 ID를 보고서 스키마의 공통 `$defs`/enum에 넣는다. facts/hypotheses/actions/prevention이 같은 정의를 참조하므로 긴 ID 목록을 네 번 반복하지 않는다. 업무 레코드 ID나 모델이 쓴 ID는 후보에 넣지 않는다. 목록이 비었거나 250개/총 9,000자를 넘으면 빈 enum이나 일부 후보 목록을 만들지 않고 기존 문자열 스키마와 서버 검증을 유지한다. 일반 조사 이력은 이 enum 제한 때문에 잘라내지 않는다. 검수 전용 입력의 CODE 선택 범위는 위 설명을 따른다. 이는 [공식 구조화 출력의 정의·enum 제한](https://developers.openai.com/api/docs/guides/structured-outputs)을 고려한 생성 보조 장치이며, 실제 저장 근거·조사 소속·인용 내용의 정확성을 대신하지 않는다. Codex 백엔드의 실제 동작은 별도 실호출로 확인한다.

공통 `InvestigationModel` 인터페이스에 두 HTTP 어댑터를 연결했다. 기존 실행기가 허용한 여덟 읽기 전용 함수만 실행하며 모델/SDK가 도구를 자체 실행하지 않는다. 일반 도구 조사의 후속 요청에 도구 call ID·서버 저장 근거·응답 output items와 암호화된 reasoning context를 보존한다. 이러한 내부 context를 보고서 근거로 인용하거나 브라우저에 노출하지 않는다.
