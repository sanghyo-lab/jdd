...
`investigation-system-v7`는 조회 묶음·간결한 보고서·항목별 직접 인용 지침을 유지하고 원인·조치·예방이 있는 보고서의 최종 인용 검수 응답 1회를 같은 한도 안에 확보하게 한다. 실행기는 구조 검증을 통과한 해당 초안을 아직 저장/공개하지 않고, 같은 모델·이력·저장 근거로 검수를 한 번 요청한다. 검수 시작 후 도구는 제공/실행하지 않는다. 수정 결과도 기존 서버 검증과 보정 횟수를 통과해야 하며 남은 호출·시간이 없으면 실패하고 근거를 보존한다. 단순 사실만 있는 보고서나 입력 부족 보고서에는 이 추가 호출을 강제하지 않는다. v1~v6 리소스도 보존한다.
...
개발 진단 로그에는 응답의 final/unphased/commentary 메시지 수와 도구 수, 최종 검수 요청의 조사 ID·반복 번호, 보고서 검증 실패의 조사 ID·모델 반복 번호·서버 정의 사유(중복 제외 최대 16개)만 남긴다. 모델 원문·근거 값·인증 정보·예외 원문은 이 로그에 넣지 않는다. 검증 기준과 오류 DTO는 그대로이며 일반 사용자 화면에 구현 정보를 추가하지 않는다.
