...
`investigation-system-v8`는 원인·조치·예방이 있는 보고서의 최종 인용 검수 응답 1회를 같은 한도 안에 확보하게 한다. 구조 검증된 초안은 아직 저장/공개하지 않고 같은 모델로 검수한다. 검수 전용 요청에는 초안·직접 인용한 CODE 원문·저장된 DATA/LOG/POLICY를 한 번씩 전달한다. 이전 모델의 추정/암호화된 reasoning 이력과 인용하지 않은 CODE 검색 구간은 검수 입력에서 제외한다. 원본 근거를 삭제하거나 내용·출처를 바꾸지 않으며 제외 범위도 요청에 명시한다. 일반 도구 조사에서는 기존 전체 대화/도구 이력을 유지한다.
...
검수 시작 후 도구는 제공/실행하지 않는다. 기존 보정 횟수·모델/도구/시간 상한·사용량/비용 장부를 유지한다. 단순 사실만 있는 보고서나 입력 부족에는 추가 검수를 강제하지 않으며 v1~v7 리소스를 보존한다.
