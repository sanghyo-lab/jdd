...
제공자는 **이상효**, 사용자는 커머스 API·재현 실행의 **김아름**과 근거 조회의 **한재홍**이다. 이 문서는 구현 기준이며 실제 API·DDL은 아직 없다. 결함의 기대 관측값은 [VOC 시나리오](voc-scenarios.md), 정상 동작은 [업무 정책](business-policy.md)을 따른다.
