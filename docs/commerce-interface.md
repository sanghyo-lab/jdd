...
제공자는 **이상효**, 사용자는 커머스 API·재현 실행의 **김아름**과 근거 조회의 **한재홍**이다. 아래 업무 API·DDL은 구현 전이며, 공통 실행 골격과 진단용 기본 테이블은 준비됐다. 결함의 기대 관측값은 [VOC 시나리오](voc-scenarios.md), 정상 동작은 [업무 정책](business-policy.md)을 따른다.
