...
## 2026-09-22 — 로컬 실행 설정과 시연 문의 보완

- 사용자 요청에 따라 로컬 전용 비밀 설정을 Git 밖에서 보존하고 Agent를 `local/codex_oauth/gpt-6-astra`로 기동했다. 사용자 제공 API 키는 로컬 호출에 사용하지 않았고 다른 PC의 인증 경로를 이 PC의 프로젝트 전용 경로로 맞췄다. 앱 build `afd717b08804-513914599664`와 web3110은 실제 PostgreSQL/HTTP에 연결된다.
- 앞선 Luna 연결 smoke1회, 모델 변경 후 Astra 연결 smoke1회가 각각 HTTP200/완료로 반환됐다. Astra 관측은 입력2957·출력234·캐시읽기0·reasoning0, 모델 호출9535ms다. 이는 CLI의 도구 없는 연결 검사이며 업무 VOC 조사·서비스 DB 모델 장부·전체 품질 성공으로 계산하지 않는다. 원문은 `runtime/local-oauth-fix/20260922T005631Z/`, `runtime/model-switch/20260922T010222Z/`에 보존했다.
- 목 데이터처럼 보이던 목록은 실제 DB에 있던 화면 검증용 문의7개였다. [커머스 기록](commerce.md)의 새 업무 재현 문의7개를 추가하고 기존 문의7·분석14를 보존했다. 실제 인증 web 중계·PC/모바일 화면에서 확인했으며 이번 조치의 조사 접수/모델 호출은0이다. 합성 데모 자료와 실제 고객 문의를 구분하고 모델 결과는 미검증으로 유지한다.
- 시작/완료 status에서 마지막 확인 이후 원격 전체 변경을 검토하고 문서 변경 main `4694b86`을 통합했다. 김아름의 직접 인수·사용자 요청 일시정지와 한재홍의 별도 시연 영상 기록을 확인했다. 위임 인계의 접수 여부는 김아름이 자기 상태에 작성한 결과를 따르며, 타인의 DONE이나 리더 최종 승인을 작성하지 않았다. 이번 공유 범위는 자기 상태 Markdown뿐이며 앱 재빌드/전체 MVP를 새로 실행한 것으로 표시하지 않는다.
