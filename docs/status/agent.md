...
## 2026-09-22T10:07:01+09:00 — 시연 v2 합성·첫 사례 실제 로그인과 새 조사 추가

- 사용자 승인에 따라 Git 밖 `output/jdd-demo/20260922-233019/revision-v2/`에 한국어 자막 포함/미포함 MP4, SRT/ASS, 새 음성, 시간표·조사 연결·검수 자료를 생성했다. 따뜻하고 차분한 한국어 여성 톤을 지시한 OpenAI `gpt-4o-mini-tts`/`marin` 음성1.0과 실제 화면1.5배를 결합했고, Java 오류 구간 7개에5px 붉은 테두리를 표시했다. v1 원본138개는 해시/크기가 모두 동일하다.
- 첫 사례는 실제 로그인→기존 문의 선택→새 조사 요청1회→공개된 조사 진행/주문 데이터·로그·실행 소스 조회→같은 조사 완료 결과를 녹화했다. 분석 `b8e73159-1342-407e-b4e4-32586effe44c`/조사 `b6a8e590-7525-45bc-96d5-96c16a76a973`의32회 관측 ID, COMPLETED/21근거, 직접 인용 소스45–70행/61행 CARD 조건을 대조했다. 이전 결과를 새 요청 결과처럼 연결하지 않았고 긴 대기 생략을 화면에 표시했다. 02~07은 출처가 보존된 기존 실제 저장 결과 재생이다.
- 새 조사는 격리된 v8 Agent build `45c879c27035-8f9e02b0545c`의 local/codex_oauth/gpt-5.6-luna이며5응답·입력54,830/출력3,452/캐시읽기1,536/reasoning1,375(출력 일부)다. 다른 개발 세션 앱/원본 업무 데이터를 재기동·수정하지 않았다. 업무 조치는 제안만 하며 실행하지 않는다.
- 이번 API 음성 합성의 명시적 사용자 승인과 $1 제작 예산을 기록했다. 요청27회 모두200/실제 WAV 저장, 최종24문장 사용이며 새 첫 사례 촬영 전의3문장은 사용하지 않고 보존했다. 누적 $0.96는 요청별 내부 배정값이며 실제 청구액이 아니다. 음성 응답에 청구 usage가 없어 미관측으로 남겼다. 일반 개발·테스트·유료 음성 인식에는 API 키를 사용하지 않았고 키/비밀번호는 산출물·Git에 노출하지 않았다.
- 두 MP4는1920×1080/H.264/yuv420p/30CFR/48kHz AAC/faststart,4,694프레임·156.466667초·7,510,400디코딩샘플이다. 프레임PTS·동일 오디오·24자막 경계·검은 빈 구간 없음·표지4초/사례 장표각2초를 기술 검사했다. 장표 PCM은 정확히0이고 AAC 경계 잔류는1 LSB 미만으로 구분한다. 새01 마지막 발화는14.3ms 이하 낮은 꼬리로 장면끝과 정합한다. workflow에는 실제 화면을 읽는 짧은 말미 여유가 있으며 정지 프레임을 덧붙이지 않았다.
- 결과 문구·같은 조사 연결·오류 박스와 화면을 분담해 직접 검수했다. 최종 선택24문장은 별도 로컬 faster-whisper base로 대조했지만 일부 전문어 인식 모호가 남아 발음 검증 PASS로 표시하지 않는다. 직접 청음과 전편 정상속도 연속 시청은 수행하지 못했으며 음색의 자연스러움까지 검수했다고 주장하지 않는다. 기존 실패/04 직접인용 한계/06 PARTIAL 범위/07 코드 경계 한계도 보존한다. 이 영상 제작은 전체 MVP·7개 첫 시도 통과·팀 DONE을 뜻하지 않으며 완료 JSON은 변경하지 않는다.
- 자료는 위 v2 경로의 `README.md`, `case-map.json`, `timeline.json`, `qa-report.md`, `first-flow/`, `audio-qa/`, `narration-input/request-ledger.json`, `edits/v2/assembly/final-media-qa.json`에 연결한다. 상태 Markdown만 협업 문서 공유 절차로 검사·일반 push하며 앱 전체 검증을 새로 수행한 것으로 표시하지 않는다.
...
- 코드01:61/CARD,02:48/경계비교,03:57–58/정수계산,04:98–100/새주문생성·저장,05:104–108/환불실패분기,06:70–97/확인한취소메서드,07:81/재고차감조건을 실제근거ID/실행소스와 대조해5px 붉은선으로 표시했다. 7개 PNG와독립시각검수통과이며06은PARTIAL범위를유지한다. 앱/보고서/업무데이터를수정하지않는다.
- v2편집은기존21문장과새음성1.0/원본화면1.5×/무음8장표를연결할전용스크립트·길이사전검사까지완료했다. 각원본은약12.25~13.52초발화를수용한다. 기존Yuna의단순변속으로개선했다고표시하지않는다.
- 준비된OpenAI TTS는gpt-4o-mini-tts/marin/따뜻하고차분한한국어여성톤지시/speed1.0/WAV다. 기존제작지침의배포API자동TTS전용금지에따라이번21문장만예상$0.10미만/$1예산사용범위를사용자에게비동기확인중이다. 로컬키존재만확인했고실제키출력/API호출0이다. 승인전API를실행하지않으며API사전검사/요청원문계획을보존한다.
- 별도venv의faster-whisper설치는137초에성공했으나base가중치다운로드가165초제한에서중단돼ASR는0회다. 부분다운로드와실패를보존하고청음/ASR완료를주장하지않는다. 최종v2영상은아직생성하지않았으며음성승인·새WAV·합본후검수를이어간다.
...
- 실제 화면 녹화 뒤 화면만1.5배, 음성1배로 편집했다. 최종 두 MP4는 1920×1080/H.264/yuv420p/30fps/48kHz AAC/faststart, 3,549프레임·118.300초·디코딩5,678,400샘플이다. 두 파일 오디오동일·전체프레임PTS·검은빈구간 없음·21자막 의미큐와 프레임/샘플 정합 검사를 통과했다. 표지4초/각장표2초 총18초는 PCM정확히0이며 AAC경계잔류 최대−113.37dBFS다.
- 선택7건의 A/B/C 화면·문구와 전환을 분담 검수했다. VOC05 녹화의 회색padding 문제는 실제viewport전체 보존 crop으로 수정했고 최초 잘못된 편집을 남겼다. 합본 첫 인코딩의 색상metadata 전환/필터재시작에 따른 프레임손실은 검사가 차단하여 실패본을 보존하고 수정 후 전체프레임을 다시 통과했다. 원본영상·이전실패·음성원본은 보존한다.
- 최종 검수 범위·프레임·해시는 `qa-report.md`, `qa/independent-final-visual-review.json`, `edits/v1/assembly/final-media-qa.json`, `artifact-sha256.json`에 제공한다. 직접청음/ASR/전편의 정상속도 연속시청은 도구로 수행하지 못했으며 해당항목은 미검수로 표시했다. 음색의 부드러움/발음·자연스러움의 청음 완료를 주장하지 않는다. 프로모션API/외부TTS API 호출0, 역할DONE·전체MVP와별개다.
- 앞선 v8 병렬 조사 인수는986b873과 통합0d536da로 main에 공유했고, 문서50개/로컬링크517개/JSON예제8개와diff검사를 통과했다. 영상·개인설정·실행로그는 Git에 넣지 않고 이 완료기록만 문서공유 절차로 검증한다. 개발 세션 앱/원본 업무데이터를 재기동·변경하지 않았다.
...
- 신규 첫 조사 결과는 01/03/04/07 COMPLETED,02/05 보고서 직접인용 검증실패,06 입력크기한도실패다. VOC02 영상은 별도 사건의 검증된 기존 v7 `e9be5f20-2d07-4996-ac51-bc3940a125a7`을 재생하며 신규실패와 연결하지 않는다. 원문47개를 기존자료/실제HTTP로 전부 대조했다. VOC04의 F3수량은 다른실제 order_items에서 참이지만 해당항목 자체의 인용공백은 남는다. 이 한계와 전체MVP 미완료를 QA에 명시하고 해설은 실제 화면이 지지하는 범위만 사용한다.
- 신규07 `3192bd87-5106-47fe-9c00-170194c394a6`는 30근거 중 직접15개·실행소스3개를 독립대조했다. 두 INVENTORY_READ의 observedQuantity=1과 최종-1/조건없는차감이 일치한다. 인용구간에 없는 트랜잭션 경계의 단정만 대본에서 뺐다. 원본3개 실제1920×1080/60fps/무음과 최종30fps/48kHz 정합을 확인했다. 음색·발음의 사람 청음은 아직 미검수다.
...
- 사용자 요청 범위인 제작 준비·프롬프트를 [실행 지시](../prompts/produce-jdd-demo-video.md)와 [7개 시나리오·정답 검수·해설 초안](../planning/jdd-demo-video-storyboard.md)으로 작성했다. 전문 PPT·영상/음성·시나리오 검수 에이전트 3개에게 읽기 전용 검토를 위임하고 결과를 통합했다. 실제 PPT/영상/음성 생성·서비스 모델/TTS 호출·과금은 수행하지 않았다.
- 실제 fixtures의 README/expected·정상 정책과 web/runner 코드를 대조했다. 8장(표지4초·케이스별2초 무음), 실제 화면 선녹화→화면1.5배속→여성 한국어 음성1.0배속→자막 순서를 명시했다. 30fps/48kHz의 프레임·샘플 경계, 의미별 cue·마지막 발화·AAC 지연 검수, 대본 수정/원본 재촬영으로 정박을 맞추는 기준을 포함한다.
- 정답지는 제작자 검수에만 사용하고 서비스 AI에 넘기지 않는다. VOC-02 동일 쿠폰 대조로 USED가 되는 오염, VOC-05/07 내부 준비 제어의 UI 부재, shop 저장 요청의 case 전환, 과거 실제 결과 재생과 새 조사 구분을 반영했다. 86d6f6a 준비 당시 VOC-01 통과/VOC-02 인용 실패/이후 PENDING과 v6 효과 미검증을 촬영 전 재확인 조건으로 남겼다.
- 도구 관측: npx·캐시 Playwright CLI0.1.21 video-start/stop 및 size/fps/cursor 옵션, macOS 한국어 음성 후보 목록은 확인했다. 범용 ffmpeg/ffprobe는 PATH에 없고 Playwright 전용 제한 빌드로 합성할 수 없다. 여성 음성의 부드러움·실제 녹화/합성 품질·폰트/장표 렌더는 아직 미검증이다.
- 문서 초안의 `python3 scripts/check_docs.py`는 50문서·498링크·8JSON 예제로 통과했다. 이후 전문가 검수에서 shop 요청 해제·VOC-01 발화 정확성·60fps 구간의3프레임 배수·발화의 샘플 배치를 보완했다. 실제 영상 제작이나 프로젝트 DONE으로 기록하지 않는다. 동시 Agent 구현 변경을 건드리지 않고 비밀 없는 별도 clone에서 이 문서 단위의 전체 공유 검증을 수행한다.
- 첫 공유 시 동시 원격 `18288a3`·`9393334`를 통합하며 상태 문서 충돌을 만났다. 제작 준비와 v6 실패/서버 인용 보완 기록을 모두 보존해 해결했고 제작 프롬프트에도 후속 실패 관측을 추가했다. 원래 실행 스택·미커밋 구현에는 손대지 않았다.
