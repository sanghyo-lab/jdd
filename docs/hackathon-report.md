...
>
> 커머스 업무와 일곱 독립 재현, Agent 접수·실행기·조회 도구·근거 저장, VOC 티켓·분석 요청의 영속 저장을 구현했다. 실제 모델 조사·VOC 전달/화면 통합과 최종 리더 승인은 진행 중이다. 아래에서 실제 업무 재현, 모의 모델 검증, 실제 AI 품질 검증을 구분한다. `미측정`과 `미검증`은 결과가 아직 없다는 뜻이다.
...
해커톤에서는 작은 이커머스 앱에 재현 가능한 장애 조건을 준비했다. 같은 조사 에이전트로 7개 문의를 분석하고, 사람이 직접 조사하는 경우와 비교해 분석 품질과 소요 시간을 평가할 계획이다. 실제 모델 평가와 사람의 조사 시간 측정은 아직 수행하지 않았다.
...
커머스는 상품 조회, 주문, 모의 결제, 쿠폰, 재고 예약·반환, 전체 취소·모의 환불을 제공한다. 에이전트는 조사 접수, 도구를 이용한 조사, 진행 상태와 분석 보고서 저장·조회를 담당한다. VOC 앱의 티켓·담당자·처리 상태·버전 충돌과 분석 요청·입력 사본·이력 저장은 구현되었다. Agent 전달·결과 조회 worker와 티켓·근거·주문 재현 화면은 담당자가 이어 구현한다. 현재 분석 요청은 PENDING에 머물며 화면은 아직 없다.

| 영역 | 현재 구현·선택 | 검증 범위 |
| --- | --- | --- |
| 백엔드 | Java 21, Spring Boot 4.1.1, Spring MVC | 전체 Gradle check·세 앱 실행 |
| AI 연결 | 공통 모델 포트와 직접 Responses HTTP/SSE 어댑터: 로컬 Codex OAuth, 배포 OpenAI API, 테스트 mock | 합성 loopback 전송·인증 경계·영속 API 비용/OAuth 관측·근거 저장; 실제 계정 접근과 품질은 미검증 |
...
[현재 실행 정책](llm-runtime.md)은 로컬 개발·영상 데모를 프로젝트 전용 Codex OAuth로, 배포를 OpenAI API로 구분한다. 일반 up/check/publish는 test/mock이며 실제 모델을 호출하지 않는다. $50 API 프로모션은 배포 전용이고 기존 누적 $30 상한을 자동 증액하지 않는다. 로컬 auth와 모델명·접근은 미준비이며 실제 OAuth/API 호출은 0회다. 공개 접속은 [ngrok 로컬 데모 절차](ngrok-local-demo.md)를 따르며 구성 선택을 실제 실행 완료로 표시하지 않는다.
...
위 그림은 목표 연결 구조다. 시연 PC에서 Next.js·Spring Boot 앱 세 개·PostgreSQL을 실행하고 ngrok로 Next.js 진입점 하나를 연결한다. Next.js는 티켓·분석 요청을 로컬 VOC로 중계하고, VOC가 내부 Agent를 호출해 조사 ID를 연결한다. 로컬 조사 작업은 `agent-app`에서 Codex OAuth를 사용하고, 배포에서는 API key를 사용한다. 둘 사이 자동 전환은 없다. 현재 세 앱·DB는 실행되며 web·전달 worker·공개 진입점은 아직 미검증이다.
...
백엔드 Gradle 모듈은 10개이며 `web`은 별도 프론트 프로젝트로 계획되어 있다. PostgreSQL은 `commerce`, `agent`, `voc` 스키마를 구분하고, 에이전트의 커머스 조회에는 SELECT 권한을 사용한다. 상세 의존성과 배포 흐름은 [구조 설계](architecture.md)와 [프론트·배포 설계](frontend-deployment.md)에 정리했다.

## 5. 에이전트의 조사 과정과 분석 리포트

다음은 목표 사용자 흐름이며, 2번의 전달·6번의 화면과 실제 모델 품질은 아직 구현·검증 중이다.
...
현재 `InvestigationRunner`가 모델 포트·8개 조회 도구·영속 근거 저장을 연결한다. 도구 결과를 DB에 저장한 뒤 모델에 전달하고, 보고서의 evidenceId를 검사한다. DB는 제한된 매개변수 조회, 로그는 상관 ID와 범위 검색, 소스와 정상 정책은 buildId별 보관본·manifest 해시 검증을 사용한다. 기본 모델은 MOCK이며 성공 보고서를 만들지 않고 설정 오류로 끝나는 실패 대역이다. 실제 OAuth/API 연결·조사 품질은 별도 검증해야 한다.

Agent의 대기 기한은 기본 10분, 수용량은 20건이며 실행 예산과 구분한다. 큐가 가득 차면 새 접수만 429로 거절하고, 같은 키는 기존 조사 ID를 돌려준다. VOC는 티켓 버전과 입력 사본을 원자적으로 저장하며 전달 재시도와 새 조사를 구분한다. 전달·조회 worker와 화면의 과부하/복구 소비는 아직 미검증이다.
...
| --- | --- | --- | --- | --- |
| VOC-01 | 3/3회 | 대조 통과 | 미검증 | `20260921T101855.977959Z-business.json` |
| VOC-02 | 3/3회 | 대조 통과 | 미검증 | `20260921T101855.977959Z-business.json` |
| VOC-03 | 3/3회 | 대조 통과 | 미검증 | `20260921T101855.977959Z-business.json` |
| VOC-04 | 3/3회 | 대조 통과 | 미검증 | `20260921T101855.977959Z-business.json` |
| VOC-05 | 3/3회 | 대조 통과 | 미검증 | `20260921T101855.977959Z-business.json` |
| VOC-06 | 3/3회 | 대조 통과 | 미검증 | `20260921T101855.977959Z-business.json` |
| VOC-07 | 20/20회 | 대조 통과 | 미검증 | `20260921T101927.342268Z-inventory.json` |

위 실행은 이상효 PC의 실제 PostgreSQL·HTTP에서 buildId `d1f92dc4ed3d-f1239ba8f94e`로 수행했다.
...
실행 가능한 커머스의 상품·주문·결제·쿠폰·재고·취소·환불, 일곱 시나리오의 합성 데이터·독립 재현과 업무 근거를 제공했다. Agent의 영속 접수·실행·조회 도구·보고서 검증과 API 비용/OAuth 관측 장부, VOC의 티켓·분석 입력 사본 저장을 제공했다. 실제 모델·VOC 전달/화면 통합과 최종 전체 검토는 남아 있다. 모든 역할과 리더는 IN_PROGRESS이며 DONE·APPROVED를 선언하지 않았다. 배포 URL과 조사 품질·시간 절감 실측치는 아직 없다.
...
| 실제 모델 준비 | [LLM 실행 안내](llm-runtime.md), [사용·비용 정책](planning/demo-llm-policy.md) | 로컬 OAuth·배포 API 분리 구현; 실제 계정/모델 미검증 |
