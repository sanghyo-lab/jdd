...
>
> 커머스 업무와 일곱 독립 재현, Agent 접수·실행기·조회 도구·근거 저장, VOC 티켓·분석 요청의 영속 저장과 Agent 전달·조회·근거 중계를 구현했다. 실제 VOC 모델 조사·화면/runner 통합과 최종 리더 승인은 진행 중이다. 아래에서 실제 업무 재현, 모의 모델 검증, 실제 AI 품질 검증을 구분한다. `미측정`과 `미검증`은 결과가 아직 없다는 뜻이다.
...
커머스는 상품 조회, 주문, 모의 결제, 쿠폰, 재고 예약·반환, 전체 취소·모의 환불을 제공한다. 에이전트는 조사 접수, 도구를 이용한 조사, 진행 상태와 분석 보고서 저장·조회를 담당한다. VOC 앱의 티켓·담당자·처리 상태·버전 충돌과 분석 요청·입력 사본·이력 저장은 구현되었다. Agent 전달·결과 조회 worker와 근거 중계는 f5064c8로 공유됐다. 저장 후 PENDING인 분석은 서버가 전달하고 상태를 갱신한다. 티켓·근거·주문 재현 화면과 runner는 아직 없다.
...
위 그림은 목표 연결 구조다. 시연 PC에서 Next.js·Spring Boot 앱 세 개·PostgreSQL을 실행하고 ngrok로 Next.js 진입점 하나를 연결한다. Next.js는 티켓·분석 요청을 로컬 VOC로 중계하고, VOC가 내부 Agent를 호출해 조사 ID를 연결한다. 로컬 조사 작업은 `agent-app`에서 Codex OAuth를 사용하고, 배포에서는 API key를 사용한다. 둘 사이 자동 전환은 없다. 현재 세 앱·DB와 전달/조회 worker는 실행된다. web·공개 진입점·실제 모델을 포함한 전체 흐름은 아직 미검증이다.
...
다음은 목표 사용자 흐름이며, 6번의 화면과 실제 VOC 모델 품질은 아직 구현·검증 중이다. 전달·조회는 backend에서 동작하며 화면 검증과 구분한다.
...
실행 가능한 커머스의 상품·주문·결제·쿠폰·재고·취소·환불, 일곱 시나리오의 합성 데이터·독립 재현과 업무 근거를 제공했다. Agent의 영속 접수·실행·조회 도구·보고서 검증과 API 비용/OAuth 관측 장부, VOC의 티켓·분석 입력 사본·전달/조회·근거 중계를 제공했다. 실제 VOC 모델·화면/runner 통합과 최종 전체 검토는 남아 있다. 모든 역할과 리더는 IN_PROGRESS이며 DONE·APPROVED를 선언하지 않았다. 배포 URL과 조사 품질·시간 절감 실측치는 아직 없다.
