...
| AI 연결 | Spring AI 2.0.1 OpenAI 전송·명시 데모 설정·도구 반복 실행기 | 모의 HTTP 단일 전송·영속 비용 한도·근거 저장; 실제 제공자 품질은 미검증 |
...
명시 데모 구성에서 worker 2와 모델 동시 호출 한도 1의 불일치도 발견해 worker를 1로 맞췄다. 이는 설정 검증이며 실제 모델 실행 결과가 아니다.
[리더 지적](status/lead-review.json) LEAD-001~013과 [커머스 상태](status/commerce.md)가 실패·수정·재검증 원문을 연결한다.
