...
> 문서 상태: 구현·검증 진행 중인 제출 초안 · 갱신일: 2026-09-22 · 이상효 위임 제출 정리, 김아름의 09:42 직접 인수 기록 추가
>
...
| 데모 URL·시연 영상 | 공개 URL 미검증. Agent 담당자가 09dffac에 영상·PPT 생성과 검수 기록 공유; 이 PC의 원본 확인·v2 최종 검수와 제출 URL은 별도 |
...
해커톤에서는 작은 이커머스 앱에 재현 가능한 장애 조건을 준비했다. 한재홍 PC의 개별 실제 모델 검수와 반복 실패를 보존했다. 최신 공유 v9 실행은 VOC-01~06 자동 검사 통과 뒤 VOC-07에서 실패했고, 별도 의미 검수에서도 일부 항목의 직접 인용 부족이 남았다. 자동 통과를 전체 품질 통과로 계산하지 않는다. 일곱 문의의 전체 품질 평가와 사람의 조사 시간 비교는 완료하지 않았고 시간 절감률은 측정하지 않았다.
...
커머스는 상품 조회, 주문, 모의 결제, 쿠폰, 재고 예약·반환, 전체 취소·모의 환불을 제공한다. 에이전트는 조사 접수, 도구를 이용한 조사, 진행 상태와 분석 보고서 저장·조회를 담당한다. VOC의 티켓·담당자·처리 상태·버전 충돌, 분석 입력 사본·이력 저장과 Agent 전달·조회·근거 중계도 구현했다. 초기 `495dbcc`의 web은 로그인·티켓 등록/목록/수정·충돌 비교·분석 이력 요약을 제공했다. 후속 통합은 조사 요청·보고서·근거 패널, `/shop`, 실제 HTTP runner와 조사별 모델 장부 조회를 제공한다. `f219cd0`은 탭 종료 뒤에도 미확인 접수의 키·버전·이전 조사 ID를 보존해 같은 입력으로 복구한다. 전체 AI 결과 검증은 남아 있다.

| 영역 | 현재 구현·선택 | 검증 범위 |
| --- | --- | --- |
| 백엔드 | Java 21, Spring Boot 4.1.1, Spring MVC | 전체 Gradle check·세 앱 실행 |
| AI 연결 | 공통 모델 포트와 직접 Responses HTTP/SSE 어댑터: 로컬 Codex OAuth, 배포 OpenAI API, 테스트 mock | 전송·장부·근거 저장 검사; 한재홍 PC 개별 검수와 v9 전체 runner 실패 보존; 김아름·리더 PC 실제 모델 미검증 |
| 저장·조회 | PostgreSQL 17.6, Spring JDBC | 실제 업무 트랜잭션·SELECT 전용 조사 계정 |
| 스키마 관리 | Flyway | 각 앱 스키마와 마이그레이션 |
| 프론트 | Next.js 16.3.5·React 19.3.0·TypeScript | 실제 PC/모바일 문의·분석 실패·커머스 흐름 및 합성 보고서/4종 근거 화면 검증; 김아름·리더 PC의 최신 실제 모델 보고서 화면은 미검증 |
...
다음 사용자 흐름의 보고서·근거 화면은 구현·공유했고 리더 위임 당시 합성 HTTP 16개를 검증했다. 이후 김아름 PC에서 합성 브라우저 19항목과 기존 공백 입력의 수정·새 키 흐름을 추가로 검증했다. 서로 다른 검사 묶음을 독립 사례 수나 실제 모델 결과로 합산하지 않는다. 한재홍 PC는 과거 실제 보고서의 새 화면을 인수했으며 현재 코드의 전체 VOC 모델 품질과 김아름·리더 PC의 실제 모델 보고서 화면 검증은 남아 있다.
...
Agent의 대기 기한은 기본 10분, 수용량은 20건이며 실행 예산과 구분한다. 큐가 가득 차면 새 접수만 429로 거절하고, 같은 키는 기존 조사 ID를 돌려준다. VOC는 티켓 버전과 입력 사본을 원자적으로 저장하며 전달 재시도와 새 조사를 구분한다. VOC 담당자는 실제 Agent의 429 네 번, 같은 키 수동 복구와 재시작 후 조회를 인수했다(06de540). 후속 합성 HTTP 화면에서 기존 입력·키 재전송과 관측 종료의 마지막 상태 보존을 확인했다. 이번 브라우저 검사에서 14분 전체를 새로 기다린 것은 아니며 최신 실제 모델 흐름은 아직 미검증이다.
...
아래 표는 일곱 결함의 리더 PC 독립 재현과 당시 인수 기록이다. 현재 전체 AI 품질 통과를 뜻하지 않으며, 이후 한재홍 PC의 v9 실행·의미 검수와 김아름 PC 직접 인수는 7절에 구분해 기록했다. 원인과 기대 결과의 상세 기준은 [시나리오 문서](voc-scenarios.md)에 있다. 이 보고서와 평가 자료는 에이전트의 조회 대상에 포함하지 않는다. 에이전트에는 문의, 식별자, 정상 업무 정책과 실제 조회 도구를 제공한다.
...
다음은 초기 실제 모델과 화면의 별도 인수 이력이다. 다른 PC의 실행을 리더 PC의 재검증으로 계산하지 않으며 최신 김아름 인수와 v9 실행 기록은 바로 아래에 별도로 적는다.
...
### 김아름 PC 직접 인수 — 2026-09-22 08:53~09:32

다음 결과는 김아름의 본 작업과 위임받은 화면·연동 검증 작업이 이 PC에서 수행했다. 원문은 Git에서 제외된 `runtime/verification/`에 보존하고 요약은 `0ddc07b`·`d635139` 및 [VOC 상태](status/voc.md)에 공유했다. 아래 횟수는 해당 검사 묶음의 관측/단언 수이며 전체 VOC 모델 평가의 성공률이 아니다.

| 대상·검증 버전 | 직접 결과 | 근거와 한계 |
| --- | --- | --- |
| 실제 HTTP·PostgreSQL, `8702c43` | 23개 통과, 08:53:27~08:53:33 KST·약6.009초 | `integration-acceptance-http-20260921T235327Z.json`; 준비 상태·관측 GET 불변/no-store·소속404·키/버전409·수정 후 옛 입력 재전송. Agent는 test/mock, OAuth/API 장부0/0 불변 |
| 분석 화면, `f219cd0`과 같은 프론트 소스 | 별도 production 복사본의 27개 소스 해시 일치·합성 HTTP 브라우저 19항목 통과 | `screen-acceptance-summary.json`; 탭 완전 종료 후 같은 키 복구·503 마지막 상태·409 초안 보존·4종 원문·오류 구분·429 기존 입력 재전송. PC1440/모바일390 가로 넘침·예상 밖 console 오류0. 실제 모델 보고서 아님 |
| 기존 공백 입력 수정 흐름 | v1 오류 안내→명시 수정·저장v2→새 키 POST1, 옛 분석 전체 불변 | `screen-acceptance-discussion-evidence.json`, `screen-acceptance-blank-before.json`, `screen-acceptance-blank-after.json`; 실제 브라우저와 합성 HTTP. 기존 실제 입력/전달 경계 검증과 결합해 [공백 입력 논의](discussions/DISC-20260921-agent-003-empty-context.md) 해소 |
| 실제 `/shop`, `f219cd0e1cbc-6eb1c1be2a83` | 12개 화면 관측과 DB 단언 통과 | `screen-acceptance-shop.json`, `screen-acceptance-shop-paid-replay.json`; 모의 결제 어댑터의 실제 HTTP/DB로 50,001−5,000=45,001원, 재전송의 동일 ID, 최종 취소 주문2·결제1·환불1·재고10. 중간·최종 snapshot을 별도 성공 건수로 합산하지 않으며 refund=null을 환불 완료로 추정하지 않음 |
| 전체 검사·3앱 실행, `4b126319e942-ea2485e8e266` | Python70·부모 준비6·web25 통과, Java229 중220통과/9조건부제외, production build·실제 DB/HTTP/SELECT 전용·근거 볼륨 검사 통과 | `agent-v9-local-integration-01.log`, `linux-gradle-check-4b126319e942-1790036452595196000/`; 일반 test/mock publish 종료0. Java 전체를 실제 PostgreSQL 검사로 계산하지 않음 |
| 프로젝트 전용 OAuth 준비, `f219cd0` | 로그인·읽기 전용 인증 연결·adapter/worker/DB 준비 확인, 전후 OAuth/API0/0 | `integration-live-prepared-20260922T001014.694819Z.json`; 이후 일반 publish는 test/mock으로 기동. 외부 전송 동의 확인 후 최신 빌드로 명시 준비·실제 모델 검증 필요 |

프론트 복구 공유 `f219cd0`의 [CI](https://github.com/sanghyo-lab/jdd/actions/runs/35670724533)와 v9·인수 기록 `0ddc07b`의 [CI](https://github.com/sanghyo-lab/jdd/actions/runs/35671452625)는 성공했다. 초기 Windows 검증 이미지의 Python/준비 모듈 누락, 오래된 `.next`의 타입 검사 실패도 보존했다. 검증 이미지에 필요한 모듈을 포함하고 깨끗한 production build로 다시 확인했으며 실패한 검사를 삭제하지 않았다.

이 PC의 실제 모델 호출은 아직0회다. VOC-01~07·NORMAL·NEEDS_INPUT·IDEMPOTENCY·RECOVERY의 live 결과 파일과 실제 모델 보고서 화면 검증은 없으며, 공개 ngrok 설정·접근 정책·URL도 준비되지 않았다. OAuth 준비·합성 화면·일반 CI 성공으로 이 항목들을 채우지 않는다.

### 다른 PC의 최신 실제 모델 기록 — 2026-09-22 09:36 공유 기준

Agent 담당자가 `368303f`로 공유한 [검수 기록](status/agent.md)에 따르면 build `4b126319e942-9152c642527f`의 v9 전체 runner는 VOC-01~06 자동 PASSED 뒤 VOC-07에서 실제 사건 LOG 미인용으로 종료1이다. 조사 자체의 COMPLETED와 평가 실패를 구분한다. 별도 의미 검수에서는 01·03·06의 항목별 직접 인용 공백도 발견했고 NORMAL·NEEDS_INPUT·IDEMPOTENCY·RECOVERY는 PENDING이다. 따라서 6/7의 품질 성공률이나 전체 완료로 환산하지 않는다.

해당 PC는 7조사·OAuth42응답과 실제 응답 모델 gpt-5.6-luna를 기록했다. 입력628,903·출력30,694·캐시읽기3,584·reasoning12,953(출력 일부), 모델 호출시간 합695,599ms는 그 실행의 관측이다. 조사 전체 시간·사람의 작업 시간·API 비용으로 바꾸지 않는다. 원문은 제공자 PC의 `runtime/mvp/20260922T002218.273964Z-92c68bba/` 및 `followup-live-runner-04*`에 있으며 김아름 PC에서 원본을 직접 검수한 자료는 아니다. 이전 실패와 미관측 usage를 보존하고 후속 개선·전체 검증을 기다린다.
...
한재홍의 관측 제공 구현을 리더 PC의 별도 PostgreSQL에서 새로 검사해 HTTP3개 모두 통과했다(21.077초/제외0). 실제 기본 서버의 기존 조사200/calls=[]·반복 동일/no-store·없는 조사404, API/OAuth0·조사13행 불변도 확인했다. `agent-observations-postgresql-current/`와 `agent-observations-running-http.json`은 합성 장부·무호출 조회 검사이며 실제 응답 모델 품질 성공이 아니다. 이후 김아름도 P1/P2를 수락하고 위 표의 실제 무호출 관측 API·준비 상태를 인수했다. 현재 모델/LOG 원문 소비와 전체 runner 검증은 남아 있다.
...
| 실제 모델 준비 | [LLM 실행 안내](llm-runtime.md), [사용·비용 정책](planning/demo-llm-policy.md), [Agent 검수 기록](status/agent.md) | 개별 검수·v9 전체 runner 실패 보존; 김아름 OAuth 준비/실제호출0, 전체 품질·리더 모델 미검증 |
| 김아름 직접 인수 | [VOC 상태](status/voc.md), `runtime/verification/` | HTTP23·합성 화면19와 추가 공백 흐름·실제shop12·전체 검사 원문 보존; 실제 모델·공개 URL 미검증 |
