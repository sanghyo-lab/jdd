...
VOC 티켓 등록·처리 상태, 분석 진행, 답변과 근거를 확인할 프론트 프로젝트 `web`을 구성한다. [김아름](roles/kim-areum-voc.md)이 VOC 서버와 프론트·AI 연동을 담당한다. 로그인·티켓 등록/목록/상세/수정·담당자/상태 필터·충돌 비교와 고정 API 중계, 분석 요청/진행/보고서/근거 및 shop 화면을 구현했다. `f219cd0`은 탭 종료 후 미확인 분석 요청의 같은 키 복구도 제공한다. [web 실행 안내](../web/README.md)를 따른다. [직접 검증 기록](status/voc.md)과 [해커톤 보고서](hackathon-report.md)에 실제 HTTP/DB·합성 화면·shop 검증을 구분했으며, 최신 실제 모델 보고서의 전체 흐름과 공개 URL 검증은 남아 있다.
