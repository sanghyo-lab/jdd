...
VOC 티켓 등록·처리 상태, 분석 진행, 답변과 근거를 확인할 프론트 프로젝트 `web`을 구성한다. [김아름](roles/kim-areum-voc.md)이 VOC 서버와 프론트·AI 연동을 담당한다. 현재 로그인·티켓 등록/목록/상세/수정·담당자/상태 필터·충돌 비교와 고정 API 중계를 구현했다. [web 실행 안내](../web/README.md)를 따른다. 아래 전체 화면 설계 중 분석 요청/진행/보고서/근거와 shop 화면·공개 URL 검증은 남아 있다.
